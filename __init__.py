...
def add_letter():
	try:
		if check_login() and session['alevel']<=2:
			...
			return render_template("message/loggedIN.html",name=session['name'],msg="Invalid request")
...
def update(id):
	try:
		...
		return render_template("message/login.html",msg="Invalid request")
def get_sd(id,data):
	try:
		dbcomm()
...
def matt():
	try:
		if check_login() and session['alevel']==3:
			...
			return render_template("message/login.html",msg="You are not authorized for this page")
	except:
		return render_template("message/login.html",msg="Invalid request")
@app.route("/timetable/")
def timt():
